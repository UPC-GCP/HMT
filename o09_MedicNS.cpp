...
double addBoundaries(Material Mat, Mesh Msh, Discretizer Dsc, ExpressionParser& Prs, int i, int j, double t, std::vector<std::vector<double>> oPhi, std::vector<std::vector<double>> obPhi){
    ...
    for (boundMain bC : Msh.boundaryConditions){
        ...
        for (size_t i = 0; i < Msh.p.N.size(); i++){
            Pos0[i] = std::lower_bound(Msh.p.Nodes[i].begin(), Msh.p.Nodes[i].end(), bC.x0[i] - Dsc.epsFind) - Msh.p.Nodes[i].begin();
            Pos1[i] = std::lower_bound(Msh.p.Nodes[i].begin(), Msh.p.Nodes[i].end(), bC.x1[i] - Dsc.epsFind) - Msh.p.Nodes[i].begin();
        }
        ...
        if (bC.type == 0){
            ...
            if (Pos0[0] == Pos1[0]){
                ...
                if (bC.side == 0){
                    // West Boundary
                    tempVal = Dsc.beta * Msh.p.matA[k].aw * (bC.value - Msh.p.Phi[i][j]) + (1 - Dsc.beta) * Msh.p.matA[k].aw * (obPhi[i][j] - oPhi[i][j]);
                } else if (bC.side == 1){
                    // East Boundary
                    tempVal = Dsc.beta * Msh.p.matA[k].ae * (bC.value - Msh.p.Phi[i][j]) + (1 - Dsc.beta) * Msh.p.matA[k].ae * (obPhi[i][j] - oPhi[i][j]);
                }
